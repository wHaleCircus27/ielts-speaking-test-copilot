...
import { AlertCircle, Database, Edit3, Loader2, Plus, RefreshCw, Save, Search, Trash2, X } from "lucide-react";
...
import type { TeacherCase, TeacherCaseInput, TeacherCaseSearchDiagnostics } from "../../types/corpus";
...
export function CorpusPage() {
  ...
  const [searchPreviewDiagnostics, setSearchPreviewDiagnostics] = useState<TeacherCaseSearchDiagnostics | null>(null);
  ...
  const [searchPreviewError, setSearchPreviewError] = useState<AppError | null>(null);
  ...
  const rebuildQueueCases = useMemo(
    () => teacherCases.filter((teacherCase) => teacherCase.embeddingStatus !== "ready"),
    [teacherCases],
  );
  const canSave =
  ...
  async function rebuildPendingAndFailedEmbeddings() {
    const targetCases = teacherCases.filter((teacherCase) => teacherCase.embeddingStatus !== "ready");
    if (!targetCases.length || queueRebuildRunning) {
      return;
    }

    setQueueRebuildRunning(true);
    setQueueRebuildProgress({ total: targetCases.length, done: 0, failures: 0 });
    setError(null);
    setNotice(null);
    let failureCount = 0;
  ...
  async function previewSearchTeacherCases() {
    ...
    try {
      setSearchPreviewDiagnostics(await diagnoseTeacherCaseSearch(normalizedQuery, 3));
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              <Button
                ...
                disabled={!rebuildQueueCases.length || queueRebuildRunning || Boolean(rebuildingCaseId) || loading}
              >
                {queueRebuildRunning ? (
          ...
          {queueRebuildRunning ? (
            <div className="mb-4 rounded-app border border-primary/20 bg-primary/10 p-3 text-xs leading-5 text-primary-strong">
              重建队列 {queueRebuildProgress.done}/{queueRebuildProgress.total}，失败 {queueRebuildProgress.failures}。
            </div>
          ) : null}
            ...
            <div className="grid gap-3">
              {teacherCases.map((teacherCase) => (
                ...
                >
                  ...
                  {teacherCase.embeddingStatus === "failed" && teacherCase.embeddingError ? (
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      ...
                      disabled={rebuildingCaseId === teacherCase.id || queueRebuildRunning}
                    >
        ...
        <Card>
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs leading-5 text-muted">
              输入题目或学生回答，查看当前阈值下会被注入评分 Prompt 的 Top-K 案例。
            </p>
          </div>
          ...
          <div className="grid gap-3">
            ...
            >
              {searchPreviewLoading ? <Loader2 size={16} className="mr-2 animate-spin" /> : <Search size={16} className="mr-2" />}
              检索 Top-K
            </Button>
            ...
            {searchPreviewDiagnostics ? (
              <div className="rounded-app border border-border bg-elevated/35 p-3 text-[11px] leading-5 text-muted">
                ...
                {formatEmbeddingSource(searchPreviewDiagnostics.embeddingSource)} · 候选{" "}
                {searchPreviewDiagnostics.readyCandidateCount} · 命中 {searchPreviewDiagnostics.matchedCount} ·
                低于阈值 {searchPreviewDiagnostics.belowThresholdCount} · {searchPreviewDiagnostics.durationMs}ms
              </div>
            ) : null}
            ...
            {searchPreviewDiagnostics?.included.length ? (
              <div className="grid gap-2">
                {searchPreviewDiagnostics.included.map((match) => (
                  <article key={match.case.id} className="rounded-app border border-border bg-elevated/35 p-3 text-xs leading-5">
                    <div className="mb-1 flex items-start justify-between gap-2">
                      <h4 className="line-clamp-1 font-semibold">{match.case.originalText}</h4>
                    ...
                    <p className="line-clamp-2 text-muted">{match.case.teacherComment}</p>
                  </article>
                ))}
              </div>
            ) : searchPreviewSearched && !searchPreviewLoading && searchPreviewQuery.trim() && !searchPreviewError ? (
            ...
            {searchPreviewDiagnostics?.nearMisses.length ? (
              <div className="grid gap-2">
                <h4 className="text-xs font-semibold text-muted">低于阈值的 near misses</h4>
                {searchPreviewDiagnostics.nearMisses.map((match) => (
                  <article key={match.case.id} className="rounded-app border border-border bg-surface/70 p-3 text-xs leading-5">
                    <div className="mb-1 flex items-start justify-between gap-2">
                      <h5 className="line-clamp-1 font-semibold">{match.case.originalText}</h5>
...
function formatSaveNotice(isUpdate: boolean, teacherCase: TeacherCase) {
  if (teacherCase.embeddingStatus === "ready") {
    return isUpdate ? "教师案例已更新，Embedding 已自动重建。" : "教师案例已保存，Embedding 已自动生成。";
  }
...
function formatEmbeddingSource(source: TeacherCaseSearchDiagnostics["embeddingSource"]) {
