...
4. 填写 Azure key、region、language。
5. 选择主题。
6. 保存配置。
...
- 智谱默认 Base URL 为 `https://open.bigmodel.cn/api/paas/v4`，Embedding 模型为 `embedding-3`，默认维度为 `1024`。
- 智谱维度限制为 `256 | 512 | 1024 | 2048`，Key 仅保存在本地配置并由 Rust command 层读取。
...
- `cd src-tauri && cargo test` 通过：30 个 Rust 测试。
- `pnpm build` 通过。
