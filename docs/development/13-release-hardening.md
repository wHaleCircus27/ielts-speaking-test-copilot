...
| --- | --- | --- | --- | --- | --- | --- |
| RH-001 | P0 | 为 asset protocol 配置、文本批改失败保留输入、历史音频隔离、endpoint 校验补充会先失败的回归测试 | 无 | 1 天 | done | `77c52a9`、`7f9b5de`；55 项前端测试与 118 项 Rust 测试覆盖四类回归 |
| RH-002 | P0 | 记录当前构建、测试、依赖审计和 Tauri 环境基线；将结果写入本文件“迭代记录” | 无 | 0.5 天 | done | 基线提交 `5c1512e` 与本文件迭代记录 |

### Phase 1：发布阻断修复

| 编号 | 优先级 | 任务 | 依赖 | 预估 | 状态 | 完成证据 |
| --- | --- | --- | --- | --- | --- | --- |
| RH-101 | P0 | 建立 `$APPDATA/generated-media` 受控目录并将新生成 WAV 写入其中；在 Tauri 配置启用 asset protocol 且 scope 仅覆盖该目录 | RH-001 | 0.5 天 | in_progress | `77c52a9`；scope、原子发布及路径测试通过，仍缺打包后的播放/Azure smoke |
| RH-102 | P0 | 实现 Azure region 严格校验，以及 DeepSeek/智谱 URL 规范化、HTTPS/本地模式策略和 Key-origin 绑定迁移 | RH-001 | 2 天 | done | `77c52a9`；恶意 URL/region、redirect、换绑及迁移事务测试通过 |
| RH-103 | P0 | 让文本批改提交显式返回成功/失败；只在成功创建历史记录后清空输入、标题和题目 | RH-001 | 0.5 天 | done | `7f9b5de`；失败、取消、并发修改保留输入，持久化成功后才清空 |
| RH-104 | P0 | 给媒体历史记录增加独立 `audioPath`，切换记录时只绑定当前记录；迁移旧 localStorage 记录 | RH-001 | 1 天 | done | `7f9b5de`；记录独立音频、旧记录降级、并发持久化与删除回滚测试通过 |

### Phase 2：安全、密钥与隐私

| 编号 | 优先级 | 任务 | 依赖 | 预估 | 状态 | 完成证据 |
| --- | --- | --- | --- | --- | --- | --- |
| RH-201 | P1 | 配置最小 CSP；为自定义 Tauri commands 建立 manifest/capability，限制可调用窗口和命令集合 | RH-101、RH-102 | 1.5 天 | done | `77c52a9`；25 个 command 显式注册，仅 `main` capability，配置校验通过 |
| RH-202 | P1 | 将三类 API Key 迁移到 macOS Keychain；非密钥配置采用 `0600`、临时文件 + `fsync` + 原子 rename | RH-102 | 2 天 | done | `77c52a9`；三 Key 事务、回读、逆序回滚、0700/0600 与持久 quarantine 测试通过 |
| RH-203 | P1 | 统一上游错误脱敏：生产错误仅保留 code、status、request ID；禁止持久化原始响应 body | RH-102 | 1 天 | done | `77c52a9`；响应限额、request ID 白名单、旧数据库及备份清理测试通过 |
| RH-204 | P1 | 在首次使用和设置页说明三条云端数据流，并提供按服务启用/停用与本地数据保留说明 | RH-203 | 1 天 | done | `77c52a9`、`7f9b5de`；默认关闭、disclosure v1、迁移提示和禁用门禁通过 |
| RH-205 | P1 | 修复生产依赖审计项并升级安全补丁；评估 Speech SDK override 兼容性，增加 Rust 依赖审计 | RH-002 | 1 天 | in_progress | `77c52a9`、`d1fa8fc`；两类 audit 与 SDK invariant 通过，仍缺真实 Azure 回归 |

### Phase 3：媒体可靠性与生命周期

| 编号 | 优先级 | 任务 | 依赖 | 预估 | 状态 | 完成证据 |
| --- | --- | --- | --- | --- | --- | --- |
| RH-301 | P1 | 将发布路径固定为 `/usr/bin/afconvert`；移除生产环境 cwd/PATH 回退；完成 clean-machine 格式兼容验证 | RH-002 | 1 天 | in_progress | `77c52a9`；绝对路径与 XML 解析已受测，仍缺 clean-machine 四格式 smoke |
| RH-302 | P1 | 增加输入大小/时长上限、受控子进程、超时/kill、有限 stderr 缓冲和取消状态 | RH-301 | 2 天 | done | `77c52a9`；资源上限、32 KiB drain、kill/wait 与幂等取消测试通过 |
| RH-303 | P1 | 在 RH-101 受控目录上实现删除记录联动、孤儿文件清理、容量上限和失败回滚 | RH-104、RH-302 | 1.5 天 | done | `77c52a9`、`7f9b5de`；路径所有权、2 GiB 上限及 reconcile/发布并发测试通过 |
| RH-304 | P1 | 为 Azure continuous recognition 增加总超时、取消和 WAV 读取大小保护，避免无限 Promise 和整文件无界内存 | RH-101、RH-302 | 1 天 | done | `77c52a9`、`7f9b5de`；64 MiB 流读取、动态超时、AbortSignal 与单次关闭测试通过 |
...
| 编号 | 优先级 | 任务 | 依赖 | 预估 | 状态 | 完成证据 |
| --- | --- | --- | --- | --- | --- | --- |
| RH-401 | P1 | 拆分 `pnpm verify` 与本地真实素材验收；用仓库内生成的小型 WAV fixture 保障 fresh clone 可运行 | RH-001 | 1 天 | done | `d1fa8fc`、`122b89f`；独立 worktree 中 pnpm/Cargo 离线安装及完整 verify 通过 |
| RH-402 | P1 | 固定 Node、pnpm、Rust 版本，建立 CI：锁文件安装、typecheck、test、build、fmt、clippy、Rust test | RH-401 | 1.5 天 | in_progress | `d1fa8fc`；版本固定与 ARM64 workflow 语法通过，尚无远端 Actions 结果 |
| RH-403 | P1 | 在 CI 加生产依赖审计、Rust advisories、secret scan，并为可接受例外建立带到期日的 allowlist | RH-205、RH-402 | 1 天 | in_progress | `d1fa8fc`；本地 audit 与当前完整历史 gitleaks 全绿，`main` required checks 尚未配置 |
| RH-404 | P1 | 配置应用图标和 `.app` bundle 构建；建立未签名内部 RC 的启动、配置、三套主题切换、文件协议和数据库迁移 smoke test | RH-201、RH-303、RH-402 | 1.5 天 | in_progress | `d1fa8fc`、`92145b5`；正式图标、arm64、ad-hoc、版本、许可资源与归档 SHA-256 已验证，仍缺 clean/upgrade `.app` smoke |
| RH-405 | P1 | 执行真实服务验收：Azure 30 秒以上音频、DeepSeek 文本评分、智谱 1024 维 benchmark 与阈值预览 | RH-304、RH-404 | 1 天 | blocked | `d1fa8fc`；DeepSeek 与智谱通过，Azure 因 `CREDENTIAL_MISSING` 未完成 |
| RH-406 | P1 | 完成发布文档：版本号、CHANGELOG、LICENSE/NOTICE、已知限制、备份/回滚步骤和 RC Go/No-Go 记录 | RH-403、RH-404、RH-405 | 1 天 | in_progress | `d1fa8fc`；版本、许可、CHANGELOG、限制与回滚文档已建，仍缺回滚演练和最终审查 |
...
| 2026-07-19 | RH-002 当前基线 | done | `5c1512ec5a10847bab78a703a8b6ab5afc29f0ae` | arm64 macOS 26.5.2；Node 24.15.0、pnpm 11.1.3、Rust/Cargo 1.95.0；typecheck、33 个前端测试、build、fmt、clippy、49 个 Rust 测试通过；官方 registry 审计发现 `ws@8.20.1` high 与 `uuid@9.0.0` moderate | CSP/ACL、有效 bundle icon、CI、fresh-clone fixture 与完整 Xcode 均缺失；默认镜像无 audit endpoint |
| 2026-07-19 | RH-001、RH-101~RH-304 代码加固 | in_progress | `77c52a9`、`7f9b5de` | `pnpm verify` 通过：13 项脚本测试、55 项前端测试、118 项 Rust 测试；配置事务和媒体生命周期并发测试全绿 | 打包后播放/Azure、clean-machine 四格式仍待验收 |
| 2026-07-19 | RH-401~RH-406 工程与发布门禁 | in_progress | `d1fa8fc`、`122b89f`、`a585693`、`92145b5`、`b035c1b` | 独立 worktree 离线 verify 通过；两类 audit 无 vulnerability 阻断；当前完整 Git 历史扫描通过；归档 `.app` 的正式图标、arm64、ad-hoc 签名、版本、许可资源和 SHA-256 正确 | Azure、`.app` smoke、branch protection、回滚与逐 RH 独立提交未满足 |
...
| 2026-07-19 | 0.1.0-rc.1 | No-Go | Azure 验收、`.app` clean/upgrade smoke、回滚演练、required checks 与逐 RH 独立提交 | 自动化、正式图标 bundle、DeepSeek、智谱和响应式浏览器预览已通过，但不得替代目标机验收 |
