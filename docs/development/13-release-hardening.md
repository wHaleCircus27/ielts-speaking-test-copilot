...
| 编号 | 优先级 | 任务 | 依赖 | 预估 | 状态 | 完成证据 |
| --- | --- | --- | --- | --- | --- | --- |
| RH-101 | P0 | 建立 `$APPDATA/generated-media` 受控目录并将新生成 WAV 写入其中；在 Tauri 配置启用 asset protocol 且 scope 仅覆盖该目录 | RH-001 | 0.5 天 | in_progress | `77c52a9`；scope、原子发布及路径测试通过，仍缺有效图标 bundle 的播放/Azure smoke |
...
| RH-403 | P1 | 在 CI 加生产依赖审计、Rust advisories、secret scan，并为可接受例外建立带到期日的 allowlist | RH-205、RH-402 | 1 天 | in_progress | `d1fa8fc`；本地 audit 与 18-commit gitleaks 全绿，`main` required checks 尚未配置 |
| RH-404 | P1 | 配置应用图标和 `.app` bundle 构建；建立未签名内部 RC 的启动、配置、三套主题切换、文件协议和数据库迁移 smoke test | RH-201、RH-303、RH-402 | 1.5 天 | blocked | `d1fa8fc`；arm64/ad-hoc/版本/许可已验证，原创完整图标与 clean/upgrade `.app` smoke 缺失 |
...
| 2026-07-19 | RH-401~RH-406 工程与发布门禁 | in_progress | `d1fa8fc`、`122b89f`、`a585693` | 独立 worktree 离线 verify 通过；两类 audit 无 vulnerability 阻断；18 个提交的 gitleaks 全历史扫描通过；归档 `.app` 的 arm64、ad-hoc 签名、版本和许可资源正确 | 图标、Azure、`.app` smoke、branch protection、回滚与逐 RH 独立提交未满足 |
...
| 2026-07-19 | 0.1.0-rc.1 | No-Go | 原创完整图标、Azure 验收、`.app` clean/upgrade smoke、回滚演练、required checks 与逐 RH 独立提交 | 自动化、DeepSeek、智谱和响应式浏览器预览已通过，但不得替代目标机验收 |
