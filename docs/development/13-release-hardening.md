...
| RH-403 | P1 | 在 CI 加生产依赖审计、Rust advisories、secret scan，并为可接受例外建立带到期日的 allowlist | RH-205、RH-402 | 1 天 | in_progress | `d1fa8fc`；本地 audit 与 15-commit gitleaks 全绿，`main` required checks 尚未配置 |
...
| 2026-07-19 | RH-401~RH-406 工程与发布门禁 | in_progress | `d1fa8fc`、`122b89f` | 独立 worktree 离线 verify 通过；两类 audit 无 vulnerability 阻断；15 个提交的 gitleaks 全历史扫描通过；arm64 `.app` 可构建且 ad-hoc 签名、版本和许可资源正确 | 图标、Azure、`.app` smoke、branch protection、回滚与逐 RH 独立提交未满足 |
